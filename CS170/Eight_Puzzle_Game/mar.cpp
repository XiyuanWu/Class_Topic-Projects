...
int calculateManhattanDistance(const vector<vector<int>>& state, const vector<vector<int>>& goalState) {
    int h_cost = 0;
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            int tile = state[i][j];
...
// A* algorithm using the Manhattan Distance heuristic
void aStarManhattan(const vector<vector<int>>& initialState) {
    ...
    frontier.push(Node(initialState, 0, calculateManhattanDistance(initialState, goalState)));
    ...
    while (!frontier.empty()) {
        ...
        for (const auto& child : children) {
            ...
            if (visited.find(childStr) == visited.end()) {
                frontier.push(Node(child, currentCost + 1, calculateManhattanDistance(child, goalState)));
