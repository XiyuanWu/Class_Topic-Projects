...
def a_star_manhattan(initial_state, goal_state):
    """
    Implements A* search using the Manhattan Distance heuristic.
    ...
    frontier = []
    initial_h_cost = manhattan_distance_heuristic(initial_state, goal_state)
    ...
    while frontier:
        ...
        for next_state, move_cost in generate_neighbors(current_state):
            ...
            new_h_cost = manhattan_distance_heuristic(next_state, goal_state)
...
def manhattan_distance_heuristic(state, goal):
    ...
    Returns:
        h_cost: The total Manhattan Distance for all tiles.
    """
    h_cost = 0
    size = len(state)
    for i in range(size):
